WITH monthly_users_recap AS (
    ...
    FROM
        {{ source('sales_database', 'order') }}
    GROUP BY
        order_month
    ORDER BY
        total_monthly_users DESC
),
...
monthly_orders_recap AS (
    SELECT
        DATE_TRUNC(order_date, month) AS order_months,
